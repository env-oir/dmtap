...
living inside a signed `pub_announce` (§22 item 3) or an already content-addressed public blob
(§22 item 2), or (b) a plain convention a CAD-aware client applies when interpreting such data.
...
(§22 public-blob profile) referenced from a `pub_announce` (§22 item 3) that carries an
`ArtifactMetadata` map. A **revision** is one such announce. An artifact's history is the
`supersedes` chain (§22 item 3, §23.5) of its revisions, each independently signed by the
publishing identity.
...
(kind `0x40`, §22 item 3, a signed plaintext CBOR announcement referencing a manifest root +
...
sealed manifests of §5.5); **author feeds** (§22 item 4, per-identity append-only monotonic-`seq`
logs); **gateway HTTP serving** (§22 item 5, plain-HTTPS feed/announce/manifest/chunk endpoints,
§23.8); and **irrevocability** (Track 1 §5.5 carve-out — a published object cannot be
...
Carried as the structured metadata of a `pub_announce` (§22 item 3) whose artifact conforms to
this profile. Like every DMTAP CBOR map, it is keyed by small unsigned integers, deterministically
encoded (§18.1.1, §18.1.2), following the same per-object-type key convention as the core wire
format even though it is not itself part of the core registry (§21).
...
| `deprecation_reason` | 9 | `tstr` | MUST iff `deprecated = true` | Human-readable reason. A `deprecated = true` announce with this field absent is malformed for this profile and SHOULD be flagged by a CAD-aware index (see §23.10, CAD-5). |
| `derived_from` | 10 | `hash` | OPTIONAL | Content address (announce id, §22 item 3) of the ancestor artifact/revision this one forks from (§23.5). Distinct from `supersedes`, which is same-identity revision history; `derived_from` is cross-identity provenance. |
...
`pub_announce` (§22 item 3), the license string is covered by the same signature as everything
...
new `pub_announce` with `supersedes` pointing at the prior one (§22 item 3, §23.5) and the new
...
**Revisions.** An artifact's history is its `supersedes` chain (§22 item 3), borrowing `edit`
...
of publishing (Track 1 §5.5 carve-out, §22 item 3): there is no operation that removes a
...
tree/DAG walk over already-content-addressed, already-integrity-checked (§22 item 2) references.
...
addressing, §22 item 2), which is in turn named from the signed `pub_announce`.
...
- **Dedup composes automatically.** Because children are content-addressed (`pin`) or resolve to
  ...
  cross-user-deduplicating (§22 item 2), extended one layer up to the artifact graph.
...
A **workshop** is purely client-side state: the set of author feeds (§22 item 4) a user follows.
...
- **Category and search indexes are derived data.** Any node — the user's own client, a community
  ...
  This mirrors §22's own posture toward indexes over feeds (§22 item 4).
- **Publishing to a workshop is publishing to your own feed.** There is no separate "workshop
  ...
  `pub_announce` to the publisher's own author feed (§22 item 4). "Adding it to a workshop" from
...
are ordinary §22 objects, so they are served exactly by §22's gateway HTTP profile (§22 item 5) —
a plain-HTTPS surface with no protocol change needed. The shapes below are **informative**
illustrations of that usage pattern; the normative endpoint grammar is §22's.

| Purpose | Illustrative shape (informative — normative grammar is §22 item 5) |
|---------|----------------------------------------------------------------------|
| Fetch a publisher's feed | `GET /pub/feed/<identity>` — paginated list of `pub_announce` ids/summaries |
| Fetch one announce | `GET /pub/announce/<announce-id>` — the signed `pub_announce` CBOR (deserialize its metadata as `ArtifactMetadata`, §23.3.1) |
| Fetch a manifest | `GET /pub/manifest/<manifest-root>` — the `Manifest`-equivalent listing chunk hashes (§22 item 2) |
| Fetch chunk bytes | `GET /pub/chunk/<chunk-hash>` — raw plaintext chunk bytes |
...
sequenceDiagram
  ...
  Client->>GW: GET chunk(s), verify each against its hash (§22 item 2)
...
holding is public on purpose (Track 1 §5.5 carve-out). Nothing this profile adds changes that
...
3. **Append to the author feed** (§22 item 4) — the publishing identity's own feed; there is no
