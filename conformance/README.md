...
`SUITE.md` catalogs **194** numbered cases and `suite.json` mirrors all **194** — the two are **in
...
`MIXPROF`/`FLEET`/`GUARD`/`LOC`/`FLOOR`/`FAILCLASS`/`GWROLE` are all mirrored) and MUST stay so.
**52 cases are byte-runnable today** (46 backed by a `vectors.json` or `pub_vectors.json` entry, 6
self-contained CBOR-reject cases whose bytes are inline); 137 are `construction-todo` — each
carries an exact construction recipe and the expected §21 error, and becomes byte-backed as its
subsystem gains a fixed-input KAT (see [Coverage vs. deferred](#coverage-vs-deferred)); 5
(`DMTAP-PUB-21`, `DMTAP-MIXPROF-01`/`-02`, `DMTAP-GWROLE-02`/`-03`) are `manual-attestation` —
client-UX, in-product-disclosure and deployment MUSTs with no wire bytes to recompute. The
partition is exact: 46 + 6 + 137 + 5 = 194.
...
2. If the case has a `vector` (or `reuses_vector`), load that entry from `vectors.json` — or, for a
   ...
   `manual-attestation` (`DMTAP-PUB-21`, `DMTAP-MIXPROF-01`/`-02`, `DMTAP-GWROLE-02`/`-03`), there
   are no wire bytes to recompute — verify the client-UX, in-product-disclosure or
   deployment/process requirement by review.
...
- **The wave-6 anti-drift families (`MIXPROF`, `FLEET`, `GUARD`, `LOC`, `FLOOR`, `FAILCLASS`,
  ...
  reproducible construction and its expected §21 code; five of them (`MIXPROF-01`/`-02`,
  `GWROLE-02`/`-03`, plus the pre-existing `PUB-21`) have **no wire bytes at all** — they are
  in-product disclosures and a process/privilege boundary — and are `manual-attestation` rather
  than a fabricated vector. Inventing bytes for those would assert a fact the protocol does not
  carry.
