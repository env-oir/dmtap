...
**Conformance coverage (honest status).** The [`conformance/`](conformance/) catalog has **344
...
vectors** in `vectors/pub_vectors.json`) plus 6 self-contained canonical-CBOR reject cases; **264
...
deployment review rather than by a runner (56 + 6 + 264 + 18 = 344). Of the (non-self-contained)
...
is **62/344 today**, growing as each subsystem gains a fixed-input KAT.
